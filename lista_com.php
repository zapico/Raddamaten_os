...
if (isset($_SESSION['user_id'])) {
    $sql = $conn->prepare("SELECT * FROM `matlista_com` where user_id=?");
...
<body>
    <div class="container w-100">
      <div class="row mx-0 pt-1">
        ...
        <a href="lista.php" class="btn btn-outline-primary col px-1 active" role="button" aria-pressed="true"><small>Min mat</small></a>
        ...
        <table class="col coltable table-striped table-sm table-bordered  ">
          <thead>
            <tr>
              ...
              <th scope="col">&nbspSlutdatum&nbsp</th>
            </tr>
          </thead>
          <tbody>
            <?php
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo "<tr class='clickable-row' data-href='add_com.php?id=" . $row["id"] . "'><td class='col'><small>" . $row["namn_matratt"] . "</small></td><td class='col-6'><small>" . $row["dateto"] . "</small></td></tr>";
    }
} else {
    echo "0 results";
}
